...
random = str(random())
class YOLO_TF:
	fromfile = 'PATH TO PICTURE' #test a picture here
	fromfolder = None
	#output_folder = 'batch1_results'  # Specify the folder path for the output files
	#output_image = f'output{random}.jpg'
	#tofile_img = os.path.join(output_folder, output_image)
	#tofile_txt = os.path.join(output_folder, 'output.txt')
	tofile_img = f'batch1_results/output{random}.jpg'
	tofile_txt = 'res/output.txt'
	imshow = True
	filewrite_img = True
	filewrite_txt = False
	disp_console = True
	...
	num_box = 1
	grid_size = 20
	classes =  ["plastic", "metal", "paper", "cardboard", "glass"]
	...
	def interpret_output(self,output):
		...
		filter_mat_boxes_arr_0 = np.array(filter_mat_boxes[0])
		filter_mat_boxes_arr_1 = np.array(filter_mat_boxes[1])
		filter_mat_boxes_arr_2 = np.array(filter_mat_boxes[2])

		# Perform indexing using arrays
		classes_num_filtered = np.argmax(filter_mat_probs, axis=3)[filter_mat_boxes_arr_0, filter_mat_boxes_arr_1, filter_mat_boxes_arr_2]
	...
	def show_results(self, img, results):
		...
		ax.imshow(cv2.cvtColor(img_cp, cv2.COLOR_BGR2RGB))
		...
		for i in range(len(results)):
			...
			confidence = results[i][5]

			if self.disp_console:
				print('    class : ' + class_name + ', [x,y,w,h]=[' + str(x) + ',' + str(y) + ',' + str(
					int(results[i][3])) + ',' + str(int(results[i][4])) + '], Confidence = ' + str(confidence))

			if self.filewrite_txt:
				ftxt.write(class_name + ',' + str(confidence) + ',' + str(x) + ',' + str(y) + ',' + str(
					int(results[i][3])) + ',' + str(int(results[i][4])) + '\n')

			# Draw red bounding box
			rect = patches.Rectangle((x - w, y - h), 2 * w, 2 * h, linewidth=1, edgecolor='r', facecolor='none')
			ax.add_patch(rect)
			ax.text(x - w, y - h - 10, class_name, fontsize=12, color='r', weight='bold')

		if self.imshow:
			plt.show()

		if self.filewrite_img:
			plt.savefig(self.tofile_img)
